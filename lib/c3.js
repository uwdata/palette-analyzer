...
c3.load = function(uri, async) {
  if (typeof XMLHttpRequest === 'undefined') {
    ...
    c3_init(JSON.parse(fs.readFileSync(uri)));
    c3_api();
    return
  }
...
function c3_api() {
  ...
  c3.terms.relatedTerms = function(w, limit) {
    var c = c3.terms.center[w], list = [];
    for (var i=0; i<W; ++i) {
      if (i != w) list.push({index: i, score: A[i*W+w]});
    }
    list.sort(function(a, b) {
      var ca, cb, cmp = b.score - a.score;
  ...
  c3.terms.center = [...Array(W).keys()].map(function(w) {
